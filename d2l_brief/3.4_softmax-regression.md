...
- Softmax function is an extension of the sigmoid function to the multiclass classification problem (see details [here](https://stats.stackexchange.com/questions/233658/softmax-vs-sigmoid-function-in-logistic-classifier).). 
...
- For a 100% certain elliptical galaxy, it's one-hot label would be y_clear = [1, 0, 0] <br>
  Entropy(y_clear) = 0 

- For a very vague galaxy image, it's one-hot label y_vague could be [0.33, 0.33, 0.33] <br>
  Entropy(y_vague) = log(3)  &emsp;   <- high entropy
...
&emsp; i.e. → Minimizing the cross-entropy loss is equivalent to minimizing the KL divergence 
 ...
 &emsp; <sub> * Nice blog post : https://towardsdatascience.com/entropy-cross-entropy-and-kl-divergence-explained-b09cdae917a
